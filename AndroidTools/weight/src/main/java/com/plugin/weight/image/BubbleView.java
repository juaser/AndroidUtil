...
public class BubbleView extends RelativeLayout {
    ...
    private int duration = 3000;//冒泡动画时间
...
//        LogUtils.e("控件宽度：" + width + "    高度：" + height);
    ...
    public void addView() {
        ...
        heartImageView.post(new Runnable() {
            @Override
            public void run() {
                ValueAnimator valueAnimator = ValueAnimator.ofObject(new BubbleEvaluator(), new PointF(width / 2, height), new PointF(width / 2, 0));
...
//            LogUtils.e("v==" + v + "   start.x==" + start.x + "   start.y==" + start.y + "   end.x==" + end.x + "  end.y==" + end.y);
            return new PointF(width / 2, (start.y - end.y) * (1 - v));
        }
    }
