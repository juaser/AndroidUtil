...
/**
 * @description： 能够测量子View长度并且如果溢出就换行
 * @author：zxl
...
public class FlowLayout extends ViewGroup {
        ...
        for (int i = 0; i < cCount; i++) {
            ...
            child.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    setCurrentselect(finalI);
                    if (onItemClickListerner != null) {
                        onItemClickListerner.click(finalI, v);
                    }
                }
            });
            MarginLayoutParams lp = (MarginLayoutParams) child
    ...
    /**
     * 根据View的选中位置，改变View的属性
     */
    public void clickToChange() {
        int cCount = getChildCount();
        for (int i = 0; i < cCount; i++) {
            PigeonTextView childAt = (PigeonTextView) getChildAt(i);
            if (i == currentselect) {
                childAt.setIsSelect(true);
            } else {
                childAt.setIsSelect(false);
            }
        }
    }
}